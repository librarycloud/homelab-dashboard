...
<template>
  ...
  <div class="settings-grid">
    ...
    <section class="settings-panel"><div class="settings-heading"><div class="settings-icon service"><el-icon><Refresh /></el-icon></div><div><h2>服务检查</h2><span>控制版本和服务状态检查频率</span></div></div><div class="setting-row"><div><strong>启用自动检查</strong><p>在服务列表中定期刷新检查结果</p></div><el-switch v-model="checking" /></div><div class="setting-row"><div><strong>检查间隔</strong><p>页面打开时使用的刷新周期</p></div><el-select v-model="checkInterval" style="width: 130px"><el-option label="每 15 分钟" value="15" /><el-option label="每 30 分钟" value="30" /><el-option label="每 1 小时" value="60" /></el-select></div><el-button @click="savePreferences">保存检查设置</el-button></section>
